...
c3.metric("Avg Days Open", round(avg_days_open,1))
c4.metric("Avg IRT (min)", round(avg_irt,1))
c5.metric("Avg Resolution Days", round(avg_resolution,1))
...
tech_fig = px.bar(
    filtered['Technology'].value_counts().reset_index(),
    x='count',
    y='Technology',
    orientation='h',
