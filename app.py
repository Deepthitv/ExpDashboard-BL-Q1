...
<style>
    ...
    /* Metric Cards - Lighter Background (#1A2C44) */
    .metric-card {{
    ...
    /* Insight Box - Lighter Background (#1A2C44) */
    .insight-box {{
        background-color: #1A2C44 !important; 
    ...
    /* RAW DATA TABLE VISIBILITY (Fix for White Font) */
    [data-testid="stTable"] {{
...
st.sidebar.download_button(label="📥 Download Raw Data (CSV)", data=csv, file_name='BankLeumi_Report.csv', mime='text/csv')
...
with col_a:
    ...
    fig_share = px.bar(filtered, x="Month", y=["Proactive", "Reactive"], color_discrete_map={"Proactive": "#4F6A8F", "Reactive": "#9AC9E3"})
...
# INITIAL RAW DATA TABLE (Explicit White Font)
...
with ins_1:
    st.markdown(f'<div class="insight-box"><h4>Engagement Strategy</h4><p>Overall proactive rate is <b>{round(filtered["Proactive_Pct"].mean())}%</b>. A clear correlation exists between increased proactive capture and lower resolution times.</p></div>', unsafe_allow_html=True)
with ins_2:
    st.markdown(f'<div class="insight-box"><h4>Efficiency Growth</h4><p>Resolution efficiency (MTTC) is trending positively. The current average is <b>{round(filtered["MTTC"].mean(), 1)} days</b>, well within operational health targets.</p></div>', unsafe_allow_html=True)
